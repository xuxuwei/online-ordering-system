										...
										<form role="form" action="backOrder!modifySuccess.action">
											<div class="form-group">
												<label>顾客名</label> <input class="form-control"
													placeholder="顾客名不能为空" value="${order.user.userName }">
											</div>
											<div class="form-group">
												<label>顾客电话</label> <input class="form-control"
													placeholder="电话不能为空" value="${order.user.userTel }">
											</div>
											<div class="form-group">
												<label>送餐地址</label> <input class="form-control"
													placeholder="地址不能为空，可考虑省市县级联实现" value="${order.orderAddress }">
											</div>
											<div class="form-group">
												<label>送餐时间</label> <input class="form-control"
													placeholder="格式如：09：30 in 2016-01-01" value="${order.orderDate }">
											</div>

								
											<div class="form-group">
												<label>其他送餐要求</label>
												<textarea class="form-control" rows="3" name="order.otherRequire">${order.otherRequire }</textarea>
											</div>
										<!-- 	<div class="form-group">
											...
											<div class="form-group">
												<label>订单总额</label> <input class="form-control" name="order.orderTotal" value="${order.orderTotal }">
											</div>
											<div class="form-group">
												<label>付款方式</label> <select class="form-control" name="order.orderPayType">
													<option value="现金" <c:if test="${order.orderPayType=='现金' }">checked="checked"</c:if> >现金</option>
													<option value="微信" <c:if test="${order.orderPayType=='微信' }">checked="checked"</c:if>>微信</option>
													<option value="支付宝" <c:if test="${order.orderPayType=='支付宝'}">checked="checked"</c:if>>支付宝</option>
												</select>
											</div>
										<%-- 	<div class="form-group">
												<label>是否配送</label>
