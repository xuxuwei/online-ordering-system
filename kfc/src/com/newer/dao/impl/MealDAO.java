...
//查询所有
	...
	public List findAlllow() {
		try {
			String queryString = "from Meal meal order by meal.mealPrice";
	         Query queryObject = getSession().createQuery(queryString);
			 return queryObject.list();
		} catch (RuntimeException re) {
			throw re;
	...
	public List findAllhigh() {
		try {
			String queryString = "from Meal meal order by meal.mealPrice desc";
	         Query queryObject = getSession().createQuery(queryString);
			 return queryObject.list();
		} catch (RuntimeException re) {
			throw re;
